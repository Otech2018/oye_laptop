...
require "../script/php/connect.php";
	...
	if( isset($_POST['reg_cus']) ){
					...
					$customer_permission = addslashes(htmlentities($_POST['customer_permission']));
						...
						$reg_customer = "Insert into customers set  customer_name='$customer_name',  customer_address='$customer_address',  customer_phone='$customer_phone',  customer_email='$customer_email',  customer_Reg_date='$reg_Date',  customer_permission='$customer_permission' ";
										
										
								
					$query_name  =new run_query("select * from customers where customer_phone='$customer_phone' ");
						if( $query_name->num_rows >= 1){
									...
									Customer  Phone Already Exists!  Do You Still Want To Register?  <hr/>
									<h1 align='center'>
									...
									<div style='position:absolute; top:0px; left:0px; z-index:8;  width:100%;height:200%; background-color:black;' id='bg'></div>"; 
										...
										$query_email  =new run_query("select * from customers where customer_address='$customer_address' ");
										if( $query_email->num_rows >= 1){
											
											echo "<div align='center' style='position:fixed; top:100px; left:35%; z-index:9; width:400px !important; padding:20px;' class='orange white-text'  id='pm'>
									Customer  Address Already Exists!  Do You Still Want To Register?  <hr/>
									<h1 align='center'>
									<form method='post'><button type='submit' name='no' class='btn red' >NO</button> &nbsp;&nbsp;&nbsp;<button type='submit' name='yes' class='btn green' >YES</button></form>
									</h1></div>
									...
									<div style='position:absolute; top:0px; left:0px; z-index:8;  width:100%;height:200%; background-color:black;' id='bg'></div>"; 
											...
											$customer_phone  =new run_query("select * from customers where customer_name='$customer_name' ");
												if( $customer_phone->num_rows >= 1){
													
													echo "<div align='center' style='position:fixed; top:100px; left:35%; z-index:9; width:400px !important; padding:20px;' class='orange white-text'  id='pm'>
									Customer  Name Already Exists!  Do You Still Want To Register?  <hr/>
									<h1 align='center'>
									<form method='post'><button type='submit' name='no' class='btn red' >NO</button> &nbsp;&nbsp;&nbsp;<button type='submit' name='yes' class='btn green' >YES</button></form>
									</h1></div>
...
<!-------
 ...
 <meta http-equiv="X-UA-Compatible" content="IE=edge">
  ...
  <meta http-equiv="pragma" content="no-cache" />
   ...
   <title> REGISTER CUSTOMER - OYEMART COMPUTERS </title>
<style>
...
?>	
   <div class="container">
   <h1 align='center' class=' tada blue-text'> REGISTER CUSTOMER</h1>
<br/>
<form method='post'>
     <div class="row">
			...
			<div class="row">
			  ...
			  <div class="row">
				...
				<div class="input-field col s4 m4">
				  ...
				  <option value="1">CREDIT WORTHY</option>
				...
				<center>
   <button type='submit' class='btn deep-orange pulse' name='reg_cus'>  ADD <i class='fa fa-plus'></i></button>
		</center>
